...
function loadConfig() {
  const defaults = {
    port: 3000, livrables_path: '../04.livrables', python_path: 'python',
    sources_path: '../03.sources',
    configuration_path: '../05.configuration', aggregators_path: '../05.configuration/aggregators.json',
    default_output_path: '../07.outputs',
...
const configurationDir = () => resolveCfg(config.configuration_path || '../05.configuration');
const aggregatorsFile = () => resolveCfg(config.aggregators_path || '../05.configuration/aggregators.json');
...
function resolveLivrablesPath() {
  ...
  const fallback = path.resolve(APP_ROOT, '..', '04.livrables');
...
let SOURCES_ROOT = resolveCfg(config.sources_path || '../03.sources');

function reloadPaths() {
  config = loadConfig();
  LIVRABLES_PATH = resolveLivrablesPath();
  SOURCES_ROOT = resolveCfg(config.sources_path || '../03.sources');
}
...
function handleBrowse(params, res) {
  ...
  let target = (pathParam && pathParam.trim()) ? pathParam : (fs.existsSync(SOURCES_ROOT) ? SOURCES_ROOT : APP_ROOT);
...
async function handleAggregatorRun(req, res) {
  ...
  const sp = path.resolve(APP_ROOT, '..', agg.script);
  if (!fs.existsSync(sp)) return sendJson(res, 404, { error: `Script agrégateur introuvable : ${agg.script}` });
  const argv = ['--input-folder', path.resolve(APP_ROOT, '..', agg.input_folder),
    '--output-file', path.resolve(APP_ROOT, '..', agg.output_file)];
...
server.listen(PORT, () => {
  ...
  console.log('Sources   : %s', SOURCES_ROOT);
