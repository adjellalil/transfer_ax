...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--prgm", type=Path, required=True,
                        help="CSV PRGM (Worldline consolide).")
    parser.add_argument("--acheteur", type=Path, required=True,
                        help="CSV ACHETEUR.")
    parser.add_argument("--parc", type=Path, required=True,
                        help="CSV PARC CLIENT (RC -> RMPM).")
    parser.add_argument("--iban-groupe", type=Path, required=True,
                        help="CSV IBAN_GROUPE (FORTIS groupe).")
    parser.add_argument("--iban-singleton", type=Path, required=True,
                        help="CSV IBAN_SINGLETON (FORTIS singleton).")
    parser.add_argument("--absorbes", type=Path, required=True,
...
def main(argv=None) -> int:
    ...
    for label, path in (
        ("--prgm", args.prgm), ("--acheteur", args.acheteur),
        ("--parc", args.parc), ("--iban-groupe", args.iban_groupe),
        ("--iban-singleton", args.iban_singleton), ("--absorbes", args.absorbes),
    ):
        if not Path(path).is_file():
            parser.error(f"{label} : fichier introuvable : {path}")
