...
def _read_duck(path, nrows=None):
    """Lecture CSV via DuckDB (tout en texte) ; None si indisponible/non-CSV/nrows."""
    import os
    if nrows is not None or os.path.splitext(str(path))[1].lower() not in (".csv", ".txt", ""):
        return None
    try:
        import duckdb
        return duckdb.read_csv(str(path), all_varchar=True).df()
    except Exception:
        return None
